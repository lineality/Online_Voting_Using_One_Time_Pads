...
Step 2: (During in-person registration) For a given single vote-ballot in a given single election (note: multiple is another design option) there need to be four physical paper documents. One "pad" (as defined here) is two identical paper documents, one for each party (two parties, the voter and the vote-office). Since there will be two "exchanges" (one where the blank-ballot is sent to the voter, and one where the filled-in-ballot is sent by the voter), there need to be two pads. (Alternately the system could be streamlined to one pad for submitting a standardized public ballot). 

which are two separate one-time-pads (or two sets of one "pad," where a one-time-pad is two identical copies of one unique "pad," with the two parties having one copy each of this unique pad.
