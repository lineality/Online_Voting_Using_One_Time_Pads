...
(Note: There is a system-design-choice to allow (or not) registration for more than one election. This issue relates to the form of the submitted ballot. For example: 
...
Notes: It may be desirable to have a 'verified public ballot' as opposed to a 'private ballot' which in theory could vary (for security and varification) from the standard public ballot (in a context of mapping out a potential attack space). 
...
# Identifying Risks 
Here we look at risks from bad-actors or bad-agents. A bad-agent may be human or automated, and may be non-local agents such as foreign states / groups.
...
Part of this process is an at-the-time randomized process of frame-shifting the ballot so that where exactly on the page each person's vote-choices appear is random. 
