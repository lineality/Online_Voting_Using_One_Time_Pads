...
Historically, a 'one time pad' was literally a pair of identical physical paper note-pads, two notebooks with the same set of characters on pages 1,2,3 etc. in both notebooks, like two copies of the same book (like two identical copies and editions of Alice in Wonderland), with the same characters on the same numbered page of each book). Each code (identical in both notebooks, both note"pad"s) was used only one time, hence the term "one-time-pad." A one-time-only used pair of identical physical paper note-pads: "one time pad"s.
...
One-time-pads are not as efficient and user friendly as 're-use-able' codes in some ways, but they are more secure and more simple. Because there is no 'key,' there is no possibility in principle to 'guess the key.' 
