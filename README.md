...
Rule 1: Problems must be solved in-person with the voter and with proof of identification in the same way that the person would register to vote and/or cast their vote.  
...
The election office, offline (enforced by software), uses the first 1:2 of the pair of printed QR codes to create an 'encrypted' version of the ballot for that one voter alone. 
...
Step 9: The voter confirms that the information in their (the voter's)  electronic scanned ballot is the same as the paper version of their (the voter's) filled out ballot. (Checking for errors.) (Note: Automated processes for checking ballots for standard mistakes.)
